...
def make_spoken_feature(input_features, text_input_list, pronounce_mapping=dict({})):
    ...
    list_pre_norm = []
    for tagging_sample, sample_word_length, text_input_features, sample_text in zip(spoken_tagging_output,
                                                                                    word_src_lengths, encoder_features,
                                                                                    text_input_list):
        spoken_feature_idx = []
                ...
                if tagging_sample[start: end].sum() > 0 and sample_words[idx] not in ['<s>', '</s>']:
                    ...
                    if (tagging_sample[start: end] == 1).sum():
                        if len(spoken_phrase) > 0:
                            norm_words.append(
                                '<mask>[{}]({})'.format(len(list_spoken_features), ' '.join(spoken_phrase)))
                            spoken_phrase = []
    ...
    if len(list_spoken_features) > 0:
        ...
        for i in range(len(list_spoken_features)):
            ...
            list_features_mask.append(torch.cat([torch.ones(spoken_length, device=device, dtype=torch.int64),
                                                 torch.zeros(remain_length, device=device,
...
def decode_plain_output(decoder_output):
    plain_output = [item.split()[1:] for item in
                    tokenizer.batch_decode(decoder_output['sequences'], skip_special_tokens=False)]
    ...
    for pre_norm in list_pre_norm:
        ...
        for w in pre_norm:
            if w.startswith('<mask>'):
                ...
                norm_val_score = None if (spoken_norm_output_score is None or threshold is None) else \
                spoken_norm_output_score[term_idx]
...
def infer(text_input_list, bias_list):
    ...
    list_spoken_features, list_features_mask, list_pre_norm = make_spoken_feature(input_raw_features, flatten_list,
                                                                                  pronounce_mapping)
    ...
    text_input_list_pre_norm = [utils.merge_chunk_pre_norm(list_chunks, overlap=20, debug=False) for list_chunks in
                                list_pre_norm_by_input]

    if len(list_spoken_features) > 0:
        spoken_norm_output, spoken_norm_score = generate_spoken_norm(list_spoken_features, list_features_mask,
                                                                     bias_features)
    else:
        spoken_norm_output, spoken_norm_score = [], None

    return reformat_normed_term(text_input_list_pre_norm, spoken_norm_output, spoken_norm_score, threshold=15,
                                debug=False)
